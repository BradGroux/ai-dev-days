# Reference Links

These are optional reference links for Brad and attendees. They are not required reading before the session.

## Event

- Eventbrite listing: https://www.eventbrite.com/e/may-2026-meeting-infragard-houston-artificial-intelligence-csc-tickets-1987707441465
- InfraGard Houston organizer page: https://www.eventbrite.com/o/infragard-houston-66950414133

## Workshop Repo

- OpenClaw Dev Days: https://github.com/BradGroux/openclaw-dev-days
- InfraGard event folder: https://github.com/BradGroux/openclaw-dev-days/tree/master/event-specific/infragard-ai-agent-workshop-2026-05-14
- Agent team project: [`../../projects/infragard-agent-team/`](../../projects/infragard-agent-team/)

## AI Risk And Critical Infrastructure

- NIST AI Risk Management Framework: https://www.nist.gov/itl/ai-risk-management-framework
- NIST AI RMF Profile on Trustworthy AI in Critical Infrastructure concept note: https://www.nist.gov/programs-projects/concept-note-ai-rmf-profile-trustworthy-ai-critical-infrastructure
- CISA Artificial Intelligence resources: https://www.cisa.gov/ai
- CISA Cross-Sector Cybersecurity Performance Goals: https://www.cisa.gov/cpg

## Agent Security And Threat Modeling

- MITRE ATLAS: https://atlas.mitre.org/
- OWASP Top 10 for Large Language Model Applications: https://owasp.org/www-project-top-10-for-large-language-model-applications/

## OpenClaw Follow-Up

- OpenClaw install guide for Mac: [`../../setup-guides/openclaw-mac.md`](../../setup-guides/openclaw-mac.md)
- OpenClaw install guide for Windows: [`../../setup-guides/openclaw-windows.md`](../../setup-guides/openclaw-windows.md)
- AI-assisted OpenClaw install: [`../../setup-guides/ai-assisted-openclaw-install.md`](../../setup-guides/ai-assisted-openclaw-install.md)
- First success lab: [`../../labs/first-success.md`](../../labs/first-success.md)
- Markdown thinking layer lab: [`../../labs/markdown-thinking-layer.md`](../../labs/markdown-thinking-layer.md)

## Community

- Start Small, Think Big Discord: https://discord.gg/Gmfkm7QVSF
